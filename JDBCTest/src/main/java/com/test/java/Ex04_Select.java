...
public class Ex04_Select {
	...
	private static void m4() {
	 ...
	 try {
			...
			rs.close();
			//여기서 .close()를 모두 날리면 2번째 SQL 실행 불가!
			//ResultSet => rs는 close 가능
			//Statement => 불가능 	계속 사용
			//Connect => 불가능 계속 사용
	...
	private static void m1() {
		...
		try {
			...
			//Result rs == 커서(cursor) > 파일 읽기, Iterator, Stream, 향상된 for, 커서
			...
			//String count = rs.getString("cnt"); //컬럼명 > 보통은 가독성으로 인해 컬럼명 사용
